...
// 윈도우 응용 프로그램의 시작 진입점
// 윈도우 응용 프로그램 실행 시 윈도우 운영체제는 이 함수를 실행해 응용 프로그램을 시작
// 이 함수가 종료되면 응용 프로그램이 종료됨
...
//
//  용도: 창 클래스를 등록합니다.
//  Window 95에서 추가된 'RegisterClassEx' 함수보다 먼저 해당코드가 Win32시스템과 호환되도록
//  하는 경우에만 이 함수를 사용.
//  이 함수를 호출해야 해당 응용프로그램에 연결된 '올바른 형식의' 작은 아이콘을 가져올 수 있음.
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    WNDCLASSEXW wcex;
    ...
    wcex.lpfnWndProc    = WndProc;   // 이 윈도우 클래스가 사용할 메시지 처리 함수(Window Procedure) 를 지정
    wcex.cbClsExtra     = 0;
    wcex.cbWndExtra     = 0;
	...
	// wcex.lpszMenuName   = NULL; // 메뉴가 없는 경우
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_LABPROJECT00);
    wcex.lpszClassName  = szWindowClass;
    wcex.hIconSm        = LoadIcon(wcex.hInstance, MAKEINTRESOURCE(IDI_SMALL));

    return RegisterClassExW(&wcex);   // 설정한 윈도우 클래스를 Windows 시스템에 등록
}
