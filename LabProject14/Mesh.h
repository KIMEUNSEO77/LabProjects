...
{
private:
	// 높이 맵 이미지 픽셀들의 2차원 배열. 각 픽셀은 0~255값
	...
	// 높이 맵 이미지를 실제로 몇 배 확대해 사용할 것인가를 나타내는 스케일 벡터
...
public:
	...
	// 높이 맵 이미지에서 (x, z) 위치의 픽셀 값에 기반한 지형의 높이를 반환
	float GetHeight(float x, float z);
	// 높이 맵 이미지에서 (x, z) 위치의 법선 벡터를 반환
...
// 높이 맵을 사용한 지형 메쉬를 표현하기 위한 클래스
class CHeightMapGridMesh : public CMesh
{
protected:
	// 격자의 크기 (가로: x축 방향, 세로: z축 방향)
	int m_nWidth;
	int m_nLength;
...
public:
	...
	// 격자의 좌표가 (x, z)일 때 교점(정점)의 높이를 반환하는 함수
	virtual float OnGetHeight(int x, int z, void* pContext);
	// 격자의 좌표가 (x, z)일 때 교점(정점)의 색상을 반환하는 함수
