...
기이다. cxQuadsPerBlock, czQuadsPerBlock은 격자 메쉬의 가로 방향과 세로 방향 사각형의 개수이다.*/
	...
	// 지형에서 가로 방향, 세로 방향으로 격자 메쉬가 몇 개가 있는 가
	long cxBlocks = (m_nWidth - 1) / cxQuadsPerBlock;
	long czBlocks = (m_nLength - 1) / czQuadsPerBlock;

	// 지형 전체를 표현하기 위한 격자 메쉬의 개수
	m_nMeshes = cxBlocks * czBlocks;

	// 지형 전체를 표현하기 위한 격자 메쉬에 대한 포인터 배열을 생성
	m_ppMeshes = new CMesh * [m_nMeshes];
	for (int i = 0; i < m_nMeshes; i++)m_ppMeshes[i] = NULL;
	...
	for (int z = 0, zStart = 0; z < czBlocks; z++)
	{
		...
		{
			// 지형의 일부분을 나타내는 격자 메쉬 시작 위치
			xStart = x * (nBlockWidth - 1);
			zStart = z * (nBlockLength - 1);

			// 지형 일부분을 나타내는 격자 메쉬를 생성해 지형 메쉬에 저장
