...
// 컴퓨터가 성능 카운터 하드웨어를 갖고 있으면 성능 카운터와 성능 주파수를 사용해 시간 단위 설정
// 성능 주파수 사용할 수 없으면 멀티미디어를 사용 - 시간 단위 0.001초로 설정
...
// 컴퓨터(CPU)가 성능 카운터 하드웨어를 갖고 있으면 성능 카운터와 성능 주파수를 사용해 현재 시간 갱신
// 없으면 멀티미디어를 사용
...
{
	// 현재 프레임 레이트를 문자열로 변환하여 lpszString 버퍼에 쓰고 FPS와 결합
	if (lpszString)
	{
